/* Edited by Emmett N. Young (4-22-14)
   1. void GraphManager::resetGraph(): resets the GraphManager::mLimit member to 0.
   2. GraphManager::GraphManager(): adds the member initialization value for
   GraphManager::mLimit member in the constructor.
   3. GraphManager::GraphManager(): adds the member initialization value for
   GraphManager::mIsExecuting member in the constructor.
   Edited by Emmett N. Young (5-26-14)
   1. Reversed the fixed_frame and base_frame.
*/
...
GraphManager::GraphManager() :
    camera_count_(ParameterServer::instance()->get<int>("camera_count")),
    optimizer_(NULL),
    ...
    mLimit(0),   // Initialize member value to 0,
    isRestrictedSendAllCloudsImpl_(false),
    pOct_(&oct1_),
    stop2DProcess_(false)
{
    ...
    optimizer_skip_ctr = new int[camera_count_];
    init_base_pose_ = new tf::Transform[camera_count_];
    latest_transform_cache_ = new tf::StampedTransform[camera_count_];
    cam_ids_to_v_ids_ = new std::map <unsigned int, unsigned int>[camera_count_];
    //keyframe_ids_ = new QList<int>[camera_count_];
    first_node_id = new unsigned int[camera_count_];
    prev_node_ = new unsigned int[camera_count_];
    next_cam_seq_id_ = new unsigned int[camera_count_];
    curr_best_result_ = new MatchingResult[camera_count_];

    //camera_vertices = new g2o::HyperGraph::VertexSet[camera_count_];
    //cam_cam_edges = new g2o::HyperGraph::EdgeSet[camera_count_];
    //current_match_edges_ = new g2o::HyperGraph::EdgeSet[camera_count_];

    for(int i=0; i<camera_count_; i++){
        optimizer_skip_ctr = 0;
        first_node_id[i] = -1;
        prev_node_[i] = -1;
        init_base_pose_[i] = tf::Transform::getIdentity();
        curr_best_result_[i] = MatchingResult();
        std::string idString = boost::lexical_cast<std::string>(i);
        std::string fixed_frame = ParameterServer::instance()->get<std::string>("fixed_frame_name");
        std::string base_frame  = ParameterServer::instance()->get<std::string>("base_frame_name"+idString);

        double t_x = ParameterServer::instance()->get<double>("cam"+idString+"_trans_x");
        double t_y = ParameterServer::instance()->get<double>("cam"+idString+"_trans_y");
        double t_z = ParameterServer::instance()->get<double>("cam"+idString+"_trans_z");
        double yaw = ParameterServer::instance()->get<double>("cam"+idString+"_yaw");
        double pitch = ParameterServer::instance()->get<double>("cam"+idString+"_pitch");
        double roll = ParameterServer::instance()->get<double>("cam"+idString+"_roll");
        /*
        init_base_pose_[i].setRotation(tf::Quaternion(tf::Vector3(1,0,0),r_x*3.14159265359/180));
        init_base_pose_[i].setRotation(tf::Quaternion(tf::Vector3(0,1,0),r_y*3.14159265359/180));
        init_base_pose_[i].setRotation(tf::Quaternion(tf::Vector3(0,0,1),r_z*3.14159265359/180));
        */
        init_base_pose_[i].setRotation(tf::Quaternion(yaw*3.14159265359/180, pitch*3.14159265359/180, roll*3.14159265359/180));
        init_base_pose_[i].setOrigin(tf::Vector3(t_x,t_y,t_z));
        latest_transform_cache_[i] = tf::StampedTransform(tf::Transform::getIdentity(), ros::Time::now(), fixed_frame, base_frame);
...
{
    int camera_id = new_node->camera_id_;
    QList<int> ids_to_link_to; //return value
    if(predecessor_id < 0) predecessor_id = prev_node_[camera_id];
    //Prepare output
    std::stringstream ss;
    ...
    { //if less prev nodes available than targets requestet, just use all
        ...
        geodesic_targets = 0;
        sampled_targets = 0;
        predecessor_id = prev_node_[camera_id];
    }
    ...
    if(sequential_targets > 0){
        ...
        int i=0;
        for(graph_revit it = graph_.rbegin(); it!=graph_.rend() && i<sequential_targets; it++){
            Node *node = it->second;
            if(node->camera_id_ == camera_id && node->id_!=predecessor_id){
                ids_to_link_to.push_back(node->id_);
                ss << ids_to_link_to.back() << ", " ;
                i++;
            }
        }*/
...
void GraphManager::resetGraph(){
...
#endif
    // Octomap 2D Part
    send2DMapMutex_.lock();
    stop2DProcess_ = true;
    isRestrictedSendAllCloudsImpl_ = false;
    // Kill the two threads first.
    f1_.waitForFinished();
    f2_.waitForFinished();
    // Reset it to true because the futures may have
    // set the isExecuting to false.
    std_srvs::Empty::Request a;
    std_srvs::Empty::Response b;
    oct1_.resetSrv(a, b);
    oct2_.resetSrv(a, b);
    mLimit = 0; // Emmett: Resets the limit of transmission of PointCloud and /tf. See
                // graph_mgr_io.cpp for more details.

    stop2DProcess_ = false;
    batch_processing_runs_ = false;
    send2DMapMutex_.unlock();
    // Octomap 2D Part
    ...
    for(int i=0; i<camera_count_; i++){
        ...
        first_node_id[i] = -1;
        prev_node_[i] = -1;
        //camera_vertices[i].clear();
        curr_best_result_[i] = MatchingResult();
        cam_ids_to_v_ids_[i].clear();
        //cam_cam_edges[i].clear();
    }
}

void GraphManager::firstNode(Node* new_node, int camera_id)
{
    ROS_INFO("START: firstNode, node: %i, cam: %i", new_node->seq_id_, camera_id);
    ...
    if(first_node_id[camera_id] != -1){
        deleteCameraFrame(first_node_id[camera_id]);
        cam_ids_to_v_ids_[camera_id].clear();
        new_node->id_ = first_node_id[camera_id];
        QList<QMatrix4x4>::iterator it_qmatrix4x4 = current_poses_.begin()+first_node_id[camera_id];
        for(QList<int>::iterator it_int = keyframe_ids_.begin(); it_int!=keyframe_ids_.end(); it_int++){
            if(*it_int == first_node_id[camera_id]){
                keyframe_ids_.erase(it_int);
                break;
            }
        }
        current_poses_.erase(it_qmatrix4x4);
        Q_EMIT deleteLastNode(camera_id);
    }
    else
        new_node->id_ = graph_.size();
    new_node->seq_id_ = next_seq_id++; // allways incremented, even if node is not added
    new_node->cam_seq_id_ = next_cam_seq_id_[camera_id]++;
    cam_ids_to_v_ids_[camera_id][new_node->cam_seq_id_] = new_node->vertex_id_;
    //init_base_pose_[camera_id] =  new_node->getGroundTruthTransform();//identity if no MoCap available
    printTransform("Ground Truth Transform for First Node", init_base_pose_[camera_id]);
    ...
    g2o::SE3Quat g2o_ref_se3 = tf2G2O(init_base_pose_[camera_id]);
    ...
    if(!ParameterServer::instance()->get<bool>("glwidget_without_clouds")) {
        Q_EMIT setPointCloud(new_node->pc_col.get(), latest_transform, camera_id);
        //Q_EMIT setFeatures(&(new_node->feature_locations_3d_));
    }
    current_poses_.append(latest_transform);
    this->addKeyframe(new_node->id_, camera_id);
    if(ParameterServer::instance()->get<bool>("octomap_online_creation")) {
        optimizeGraph(camera_id); //will do the following at the end:
        //updateCloudOrigin(new_node);
        //renderToOctomap(new_node);
    }
    first_node_id[camera_id] = new_node->id_;
    prev_node_[camera_id] = new_node->id_;
    process_node_runs_ = false;
    ROS_INFO("END: firstNode, node: %i, cam: %i", new_node->seq_id_, camera_id);
}
...
bool GraphManager::nodeComparisons(Node* new_node, 
                                   ...
                                   bool& edge_to_keyframe, int camera_id)///Output:contains the best-yet of the pairwise motion estimates for the current node

{
    /// \callergraph
    ScopedTimer s(__FUNCTION__);
    ROS_INFO("START: nodeComparisons, node: %i, cam: %i", new_node->id_, camera_id);
            ...
            ! ps->get<bool>("keep_all_nodes"))
    {
        ...
        process_node_runs_ = false;
        ROS_INFO("END: nodeComparisons, node: %i, cam: %i, result: FALSE", new_node->id_, camera_id);
        return false;
    }
    ...
    //Odometry Stuff
    int sequentially_previous_id = prev_node_[camera_id];
    ...
    MatchingResult mr;
    curr_best_result_[camera_id] = mr;
            ...
            ps->get<double>("min_rotation_degree") > 0.0)
    {
        ...
        Node* prev_frame = graph_[prev_node_[camera_id]];

        ROS_INFO("OK PA1-1, prev_frame: %i",prev_node_[camera_id]);
        //if(localization_only_ && curr_best_result_[camera_id].edge.id1 > 0){ prev_frame =  graph_[curr_best_result_[camera_id].edge.id1]; }
        ...
        if(mr.edge.id1 > 0 && mr.edge.id2 > 0) {//Found trafo
            ...
            if(!isBigTrafo(mr.edge.mean) || !isSmallTrafo(mr.edge.mean, delta_time.toSec())){ //Found trafo, but bad trafo (too small to big)
                ...
                latest_transform_cache_[camera_id] = stampedTransformInWorldFrame(new_node, combined, camera_id);
                printTransform("Computed new transform", latest_transform_cache_[camera_id]);
                broadcastTransform(latest_transform_cache_[camera_id]);
                process_node_runs_ = false;
                curr_best_result_[camera_id] = mr;
                ROS_INFO("END: nodeComparisons, node: %i, cam: %i, result: FALSE2", new_node->id_, camera_id);
                return false;
                ...
                if (addEdgeToG2O(mr.edge, prev_frame, new_node,  true, true, curr_motion_estimate))
                {
                    ...
                    process_node_runs_ = false;
                    ROS_INFO("END: nodeComparisons, node: %i, cam: %i, result: FALSE3", new_node->id_, camera_id);
                    return false;
                }
            }
    ...
    if(predecessor_matched){
        vertices_to_comp = getPotentialEdgeTargetsWithDijkstra(new_node, seq_cand, geod_cand, samp_cand, curr_best_result_[camera_id].edge.id1);
    } else {
    ...
    if (ps->get<bool>("concurrent_edge_construction"))
    {
        ...
        for (int i = 0; i < results.size(); i++)
        {
            ...
            if (mr.edge.id1 >= 0 ) {
                ...
                if (isSmallTrafo(mr.edge.mean, delta_time.toSec()) &&
                        addEdgeToG2O(mr.edge,graph_[mr.edge.id1],new_node, isBigTrafo(mr.edge.mean), mr.inlier_matches.size() > curr_best_result_[camera_id].inlier_matches.size(), curr_motion_estimate))
                {
                    graph_[new_node->id_] = new_node; //Needs to be added
#ifdef DO_FEATURE_OPTIMIZATION
                    ...
                    graph_[mr.edge.id1]->valid_tf_estimate_ = true;
                    ROS_INFO("Added Edge between %i and %i. Inliers: %i",mr.edge.id1,mr.edge.id2,(int) mr.inlier_matches.size());
                    if (mr.inlier_matches.size() > curr_best_result_[camera_id].inlier_matches.size()) {
                        curr_best_result_[camera_id] = mr;
                    }
                    if(keyframe_ids_.contains(mr.edge.id1)) edge_to_keyframe = true;
                }
        ...
        for (int id_of_id = (int) vertices_to_comp.size() - 1; id_of_id >= 0; id_of_id--) {
            ...
            //if(node_to_compare->camera_id_ != camera_id) continue;
            ...
            if (mr.edge.id1 >= 0) {
                ...
                ros::Duration delta_time = new_node->pc_col->header.stamp - graph_[mr.edge.id1]->pc_col->header.stamp;
                if (isSmallTrafo(mr.edge.mean, delta_time.toSec()) &&
                        addEdgeToG2O(mr.edge, node_to_compare, new_node, isBigTrafo(mr.edge.mean), mr.inlier_matches.size() > curr_best_result_[camera_id].inlier_matches.size(), curr_motion_estimate))
                {
#ifdef DO_FEATURE_OPTIMIZATION
    ...
    */
    ...
    ROS_INFO("END: nodeComparisons, node: %i, cam: %i, result: %i", new_node->id_, camera_id, cam_cam_edges.size() > num_edges_before);
...
void GraphManager::localizationUpdate(Node* new_node, QMatrix4x4 motion_estimate)
{
    ...
    Q_EMIT setPointCloud(cloud_to_visualize, motion_estimate, camera_id_);
...
bool GraphManager::addNode(Node* new_node, int camera_id)
{
    ScopedTimer s(__FUNCTION__);
    ROS_INFO("START: addNode, node: %i, cam: %i", new_node->seq_id_, camera_id);
    ...
    if (first_node_id[camera_id] == -1){
        firstNode(new_node, camera_id);
        ROS_INFO("END: addNode, node: %i, cam: %i", new_node->seq_id_, camera_id);
        return true;
    }
    ...
    bool found_match = nodeComparisons(new_node, motion_estimate, edge_to_last_keyframe_found, camera_id);
        ...
        */
        {
            ...
            if(!edge_to_last_keyframe_found && earliest_loop_closure_node_ > keyframe_ids_.back()) {
                this->addKeyframe(new_node->id_-1, camera_id);//use the id of the node before, because that one is still localized w.r.t. a keyframe. So keyframes are connected
            } else {
            ...
            Q_EMIT setPointCloud(cloud_to_visualize, motion_estimate, camera_id);
            Q_EMIT setFeatures(features_to_visualize);
            ...
            {
                if(optimizer_skip_ctr[camera_id] > ParameterServer::instance()->get<int>("optimizer_first_skip"))
                    optimizeGraph(camera_id);
                else
                    optimizer_skip_ctr[camera_id]++;
            }
    ...
    else //Unsuccesful
    {
        ...
        if(cam_ids_to_v_ids_[camera_id].size() == 1 && new_node->feature_locations_2d_.size() > graph_[first_node_id[camera_id]]->feature_locations_2d_.size()){//if there is only one node which has less features, replace it by the new one
            ROS_WARN("Choosing new initial node for cam %i, because it has more features", camera_id);
            //this->resetGraph();
            process_node_runs_ = false;
            firstNode(new_node, camera_id);
            return true;
    ...
    Q_EMIT setGUIInfo(message.sprintf("%s, Camera Pose Graph Size: %iN/%iE, Duration: %f, Inliers:%5i",// &chi;<sup>2</sup>: %f",
                                      found_match ? "Added" : "Ignored", (int)camera_vertices.size(), (int)cam_cam_edges.size(), s.elapsed(), (int)curr_best_result_[camera_id].inlier_matches.size()));//, optimizer_->chi2()));
    process_node_runs_ = false;
...
void GraphManager::addKeyframe(int id, int camera_id)
{
    ScopedTimer s(__FUNCTION__);
...
double GraphManager::optimizeGraph(int camera_id, double break_criterion, bool nonthreaded, QString filebasename){
    if(ParameterServer::instance()->get<bool>("concurrent_optimization") && !nonthreaded) {
        ROS_DEBUG("Optimization done in Thread");
        QtConcurrent::run(this, &GraphManager::optimizeGraphImpl, break_criterion, camera_id);
        return -1.0;
    }
    else { //Non-concurrent
        return optimizeGraphImpl(break_criterion, camera_id);//regular function call
    }
}

void GraphManager::fixationOfVertices(std::string strategy,
    ...
    if (strategy == "previous" && graph.size() > 2) {
        ...
        //for(int i=0; i<ParameterServer::instance()->get<int>("camera_count"); i++)
        //    optimizer->vertex(graph[first_node_id[i]]->vertex_id_)->setFixed(true);
    }
}

double GraphManager::optimizeGraphImpl(double break_criterion, int camera_id)
{
    ScopedTimer s(__FUNCTION__);
    ParameterServer* ps = ParameterServer::instance();
    ...
    latest_transform_cache_[camera_id] = stampedTransformInWorldFrame(newest_node, computed_motion_, camera_id);
    ...
    Q_EMIT setGraphEdges(getGraphEdges(camera_id), camera_id);
    Q_EMIT updateTransforms(getAllPosesAsMatrixList(camera_id), camera_id);
...
unsigned int GraphManager::pruneEdgesWithErrorAbove(float thresh, int camera_id){
    QMutexLocker locker(&optimizer_mutex_);
    QMutexLocker locker2(&optimization_mutex_);
...
QList<QPair<int, int> >* GraphManager::getGraphEdges(int camera_id)
{
    ScopedTimer s(__FUNCTION__);
...
QList<QMatrix4x4>* GraphManager::getAllPosesAsMatrixList(int camera_id){
    ScopedTimer s(__FUNCTION__);
...
void GraphManager::printEdgeErrors(QString filename, int camera_id){
    QMutexLocker locker(&optimizer_mutex_);
    std::fstream filestr;
...
void GraphManager::sanityCheck(float thresh, int camera_id){
