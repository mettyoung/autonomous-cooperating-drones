...
Graphical_UI::Graphical_UI() : filename("quicksave.pcd"), glviewer(NULL)
{
    infoText = new QString(tr(
                "<p><b>MULTI-RGBDSLAM</b> is a modified version of RGBDSLAM by Endres, Felix et. al "
                "extended to support multiple cameras.</p>"
                ...
                "the RGBD-images and compute a globally consistent 6D trajectory."
                "<break><a href=\"http://wiki.ros.org/rgbdslam/\">http://wiki.ros.org/rgbdslam</a></p>"
                ""));
    licenseText = new QString(tr(
                 "<p><b>MULTI-RGBDSLAM</b> is free software: you can redistribute it and/or modify"
                 "it under the terms of the GNU General Public License as published by"
                 "the Free Software Foundation, either version 3 of the License, or"
                 "(at your option) any later version.</p>"
                 "<p><b>MULTI-RGBDSLAM</b> is distributed in the hope that it will be useful,"
                 "but WITHOUT ANY WARRANTY; without even the implied warranty of"
                 "MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the"
                 "GNU General Public License for more details.</p>"
                 "<p>You should have received a copy of the GNU General Public License"
                 "along with <b>MULTI-RGBDSLAM</b>.  If not, refer to <a href=\"http://www.gnu.org/licenses/\">http://www.gnu.org/licenses</a>.</p>"));
    ...
    pause_on = ParameterServer::instance()->get<bool>("start_paused");
    // setup the layout:
    ...
    int i;
    //QSplitter** hsplitter = new QSplitter*[camera_count];
    QSplitter* hsplitter = new QSplitter();
    //feature_flow_image_label = new QLabel*[camera_count];
    visual_image_label = new QLabel*[camera_count];
    //depth_image_label = new QLabel*[camera_count];
    for(i = 0; i < camera_count; i++){
        /*
        feature_flow_image_label[i] = new QLabel(*mouseHelpText);
        // create widgets for image and map display
        feature_flow_image_label[i]->setWordWrap(true);
        feature_flow_image_label[i]->setFrameStyle(QFrame::StyledPanel | QFrame::Sunken);
        if(ParameterServer::instance()->get<bool>("scalable_2d_display"))
          feature_flow_image_label[i]->setScaledContents(true);
        */
        visual_image_label[i] = new QLabel("<i>Waiting for monochrome image...</i>");
        visual_image_label[i]->setFrameStyle(QFrame::StyledPanel | QFrame::Sunken);
        visual_image_label[i]->setAlignment(Qt::AlignCenter);
        if(ParameterServer::instance()->get<bool>("scalable_2d_display"))
          visual_image_label[i]->setScaledContents(true);
        /*
        depth_image_label[i] = new QLabel(tr("<i>Waiting for depth image...</i>"));
        depth_image_label[i]->setFrameStyle(QFrame::StyledPanel | QFrame::Sunken);
        depth_image_label[i]->setAlignment(Qt::AlignCenter);
        if(ParameterServer::instance()->get<bool>("scalable_2d_display"))
          depth_image_label[i]->setScaledContents(true);
        */
        // arrange image labels in horizontal layout
        hsplitter->addWidget(visual_image_label[i]);
        //hsplitter[i]->addWidget(depth_image_label[i]);
        //hsplitter[i]->addWidget(feature_flow_image_label[i]);

    }
    twod_image_label = new QLabel("<i>Waiting for 2D Map...</i>");
    twod_image_label->setFrameStyle(QFrame::StyledPanel | QFrame::Sunken);
    twod_image_label->setAlignment(Qt::AlignCenter);
    if(ParameterServer::instance()->get<bool>("scalable_2d_display"))
      twod_image_label->setScaledContents(true);
    hsplitter->addWidget(twod_image_label);
    ...
    QString message = tr("Ready for MULTI-RGBDSLAM");
    ...
    setWindowTitle(tr("MULTI-RGBDSLAM"));
    setMinimumSize(790, 290);
    resize(1000, 700);
}

void Graphical_UI::set2DMap(QImage qimage){
  if(twod_image_label->isVisible()){
      twod_image_label->setAlignment(Qt::AlignCenter);
      twod_image_label->setPixmap(QPixmap::fromImage(qimage));
      twod_image_label->repaint();
  }
}
void Graphical_UI::setFeatureFlowImage(QImage qimage, int camera_id){/*
  if(feature_flow_image_label[camera_id]->isVisible()){
      feature_flow_image_label[camera_id]->setAlignment(Qt::AlignCenter);
      feature_flow_image_label[camera_id]->setPixmap(QPixmap::fromImage(qimage));
      feature_flow_image_label[camera_id]->repaint();
  }*/
  if(!pause_on)
      if(visual_image_label[camera_id]->isVisible()){
          visual_image_label[camera_id]->setAlignment(Qt::AlignCenter);
          visual_image_label[camera_id]->setPixmap(QPixmap::fromImage(qimage));
          visual_image_label[camera_id]->repaint();
      }
}

void Graphical_UI::setRGBDImage(QImage rgb, QImage depth, int camera_id){
    if(pause_on)
        if(visual_image_label[camera_id]->isVisible()){
          visual_image_label[camera_id]->setPixmap(QPixmap::fromImage(rgb));
          visual_image_label[camera_id]->repaint();
        }
    /*
    if(depth_image_label[camera_id]->isVisible()){
        depth_image_label[camera_id]->setPixmap(QPixmap::fromImage(depth));
        depth_image_label[camera_id]->repaint();
    }
    */
}
...
void Graphical_UI::setCameraFollow() {
  this->setParam("cam_follow_num");
}
...
void Graphical_UI::about() {
    QMessageBox::about(this, tr("About MULTI-RGBDSLAM"), *infoText + *licenseText);
}
...
void Graphical_UI::set2DStream(bool is_on) {
    ...
    if(is_on){
        for(i = 0; i < camera_count; i++){
            visual_image_label[i]->show();
            //depth_image_label[i]->show();
            //feature_flow_image_label[i]->show();
        }
        QList<int> list;
        ...
        for(i = 0; i < camera_count; i++){
            visual_image_label[i]->hide();
            //depth_image_label[i]->hide();
            //feature_flow_image_label[i]->hide();
        }
        QList<int> list;
        list.append(1);//upper part on
...
void Graphical_UI::keyPressEvent(QKeyEvent* e)
{
      Q_EMIT FPS_press(e);
};
...
*/
    ...
    */
    ...
    if(ParameterServer::instance()->get<bool>("use_glwidget"))
    {
      ...
      QAction *setCameraFollowAct = new QAction(tr("Camera to Follow"), this);
      //setOctoMapResolutionAct->setShortcuts(QKeySequence::SaveAs);
      setCameraFollowAct->setStatusTip(tr("Change the octomap resolution. Clears previously created maps on next update."));
      connect(setCameraFollowAct, SIGNAL(triggered()), this, SLOT(setCameraFollow()));
      viewMenu->addAction(setCameraFollowAct);
      this->addAction(setCameraFollowAct);
