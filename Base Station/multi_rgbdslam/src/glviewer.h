...
public Q_SLOTS:
    ...
    void FPS_press(QKeyEvent *key);

    //Formerly called trough qt_gui
    void addPointCloud(pointcloud_type * pc, QMatrix4x4 transform, int camera_id);
    void addFeatures(const std::vector<Eigen::Vector4f, Eigen::aligned_allocator<Eigen::Vector4f> >* feature_locations_3d);
    void updateTransforms(QList<QMatrix4x4>* transforms, int camera_id);
    void setEdges(const QList<QPair<int, int> >* edge_list, int camera_id);
...
protected:
    ...
    void pointCloud2GLList(pointcloud_type const * pc, int camera_id);
    void pointCloud2GLPoints(pointcloud_type * pc, int camera_id);
    void pointCloud2GLStrip(pointcloud_type * pc, int camera_id);
...
private:
    ...
    float FPS_xTra, FPS_yTra, FPS_zTra;
    QPoint lastPos;
    GLenum polygon_mode;
    ...
    QMatrix4x4 viewpoint_tf_, FPS_viewpoint_tf_;
    QMatrix4x4* init_camera_tf;
