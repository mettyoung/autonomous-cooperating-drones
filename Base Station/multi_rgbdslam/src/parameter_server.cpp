...
void ParameterServer::defaultConfig() {
  ...
  // Input data settings
  //RIGEL
  addOption("camera_namespace",              std::string("/camera"),                    "Generic namespace is to be followed by a number and concatenated in front of the camera default topics if no specific topics are given");
  addOption("camera_count",                  static_cast<int> (1),                      "Number of cameras to be used in RGBDSLAM");
  addOption("topic_image_mono",              std::string("/rgb/image_rect_color"),      "Color or grayscale image ros topic");
  addOption("camera_info_topic",             std::string("/rgb/camera_info"),           "Required for backprojection if no pointcloud topic is given");
  addOption("topic_image_depth",             std::string("/depth_registered/image_rect"),        "Depth image ros topic");
  addOption("optimizer_first_skip",          static_cast<int> (10),                     "");
  getValues();


  //RIGEL
  std::string camera_ns = get<std::string>("camera_namespace");
  std::string topic_image_mono = get<std::string>("topic_image_mono");
  std::string camera_info_topic = get<std::string>("camera_info_topic");
  std::string topic_image_depth = get<std::string>("topic_image_depth");
  std::string camera_rgb_optical_ns = "_rgb_optical_frame";
  int camera_count = get<int>("camera_count");
  int i;
  std::string iString;

  for(i=0; i<camera_count; i++){
      iString = boost::lexical_cast<std::string>(i);
      addOption("cam"+iString+"_trans_x",                 static_cast<double> (0),                        "Translation X");
      addOption("cam"+iString+"_trans_y",                 static_cast<double> (0),                        "Translation Y");
      addOption("cam"+iString+"_trans_z",                 static_cast<double> (0),                        "Translation Z");
      addOption("cam"+iString+"_yaw",                     static_cast<double> (0),                        "Yaw");
      addOption("cam"+iString+"_pitch",                   static_cast<double> (0),                        "Pitch");
      addOption("cam"+iString+"_roll",                    static_cast<double> (0),                        "Roll");
      addOption("topic_image_mono"+iString,               camera_ns + iString + topic_image_mono,      "Color or grayscale image ros topic");
      addOption("camera_info_topic"+iString,              camera_ns + iString + camera_info_topic,     "Required for backprojection if no pointcloud topic is given");
      addOption("topic_image_depth"+iString,              camera_ns + iString + topic_image_depth,     "Depth image ros topic");
      // Octomap 2D Server Settings (Memetts)
      addOption("base_frame_id"+iString,                  camera_ns + iString + camera_rgb_optical_ns, "Base Frame ID for Octomap 2D");
  }
  //Most of the original options for the input has been removed to getValues() to be able to support multiple cameras.
  //ENDRIGEL

  addOption("cam_follow_num",                static_cast<int> (0),                      "");
  ...
  addOption("world_frame_name",              std::string("/world"),                     "");
  addOption("fixed_frame_name",              std::string("/map"),                       "The computed camera transforms are with respect to this frame. It is set to the identity for the first camera's first frame processed or, if ground truth is available, to the ground truth of the first frame");

  for(i = 0; i < camera_count; i++){
    iString = boost::lexical_cast<std::string>(i);
    addOption("base_frame_name"+iString,     std::string("/camera"+iString+"_link"),    "If the camera is articulated use robot base");
  }
  ...
  addOption("scalable_2d_display",           static_cast<bool> (true),                  "Whether the input images are expanded. Consumes CPU time");
...
void ParameterServer::checkValues() {
    ...
    //RIGEL
    if (get<int>("camera_count") <= 0){
        config["camera_count"] = 1;
        ROS_WARN("Camera count was found to be set to be invalid! Defaulting to 1 camera.");
    }
    //ENDRIGEL
}
