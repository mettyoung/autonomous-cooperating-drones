...
GLViewer::GLViewer(QWidget *parent)
    : QGLWidget(QGLFormat(QGL::SampleBuffers|QGL::StereoBuffers), parent),
      ...
      cloud_list_indices(new QList<GLuint>[ParameterServer::instance()->get<int>("camera_count")]),
      cloud_matrices(new QList<QMatrix4x4>*[ParameterServer::instance()->get<int>("camera_count")]),
      init_camera_tf(new QMatrix4x4[ParameterServer::instance()->get<int>("camera_count")]),
      edge_list_(new QList<QPair<int, int> >[ParameterServer::instance()->get<int>("camera_count")]),
      ...
      fast_rendering_step_(0)
{
    for(int i = 0; i < camera_count_; i++){
        cloud_matrices[i] = new QList<QMatrix4x4>();
        init_camera_tf[i].setToIdentity();
    }
    ...
    FPS_viewpoint_tf_.setToIdentity();
    FPS_viewpoint_tf_.translate(0,0,-50);
...
void GLViewer::drawClouds(float xshift) {
    ...
    if(follow_mode_){
        int id = cloud_matrices[ParameterServer::instance()->get<int>("cam_follow_num")]->size()-1;
        if(id >= 0)setViewPoint((*cloud_matrices[ParameterServer::instance()->get<int>("cam_follow_num")])[id]);
        //FPS_viewpoint_tf_ = viewpoint_tf_.inverted();
    }
    ...
    step = std::max(step,1);
    for(int j = 0; j < camera_count_; j++){
        glMultMatrixd(static_cast<GLdouble*>( init_camera_tf[j].data() ));
        int last_cloud = std::min(cloud_list_indices[j].size(), cloud_matrices[j]->size());
        for(int i = 0; i < last_cloud; i+=step){
            glPushMatrix();
            glMultMatrixd(static_cast<GLdouble*>( (*cloud_matrices[j])[i].data() ));//works as long as qreal and GLdouble are typedefs to double (might depend on hardware)
            if(show_clouds_) glCallList(cloud_list_indices[j][i]);
    ...
    //}
        ...
        if(show_edges_) drawEdges(j);
    //for(int j = 0; j < camera_count_; j++){
        //glMultMatrixd(static_cast<GLdouble*>( init_camera_tf[j].data() ));
        for(int i = 0; i<cloud_list_indices[j].size() && i<cloud_matrices[j]->size(); i++){
            glPushMatrix();
            glMultMatrixd(static_cast<GLdouble*>( (*cloud_matrices[j])[i].data() ));//works as long as qreal and GLdouble are typedefs to double (might depend on hardware)
            if(show_poses_) drawAxis((i + 1 == cloud_list_indices[j].size()) ? 0.5:0.075); //Draw last pose Big
            if(i + 1 == cloud_list_indices[j].size())
                drawQuad();
            if(show_ids_) {
...
void GLViewer::drawQuad(){
    float size = 0.45;
    float arm_height = 0.01;
    float arm_width = 0.03;
    ...
    for(int i=0; i<4; i++){
        ...
        glColor4f(r, g, b, a);
        glVertex3f( arm_width/2, arm_height/2, +arm_width/2);
        glVertex3f( -arm_width/2, arm_height/2, +arm_width/2);
        glVertex3f( -arm_width/2,  arm_height/2, size/2);
        glVertex3f( arm_width/2,  arm_height/2, size/2);
        ...
        glTranslatef(0,-arm_height/2,size/2);
        glRotatef(90.0f, 1.0f, 0.0f, 0.0f);
        gluCylinder(quadratic,0.1f,0.1f,0.01f,32,32);
        glRotatef(180.0f, 1.0f, 0.0f, 0.0f);

        glBegin(GL_POLYGON);
        for(double i = 0; i < 2 * PI; i += PI / 300) //<-- Change this Value
            glVertex3f(cos(i) * .1f, sin(i) * .1f, 0.0);
        glEnd();
...
void GLViewer::mouseDoubleClickEvent(QMouseEvent *event) {
    ...
    xTra=0;
    yTra=0;
    if(cloud_matrices[0]->size()>0){
      int id = 0;
      switch (QApplication::keyboardModifiers()){
        case Qt::NoModifier:
            id = cloud_matrices[0]->size()-1;
        case Qt::ControlModifier:
          setViewPoint((*cloud_matrices[0])[id]);
            break;
...
void GLViewer::FPS_press(QKeyEvent *key){
    switch(key->key()){
        case Qt::Key_W:
            FPS_viewpoint_tf_.translate(0,0,NUMBERKO);
            //FPS_zTra+=NUMBERKO;
            //FPS_viewpoint_tf_.translate(FPS_xTra,FPS_yTra,FPS_zTra);
            break;
        case Qt::Key_A:
            FPS_viewpoint_tf_.translate(-NUMBERKO,0,0);
            //FPS_xTra-=NUMBERKO;
            //FPS_viewpoint_tf_.translate(FPS_xTra,FPS_yTra,FPS_zTra);
            break;
        case Qt::Key_S:
...
void GLViewer::updateTransforms(QList<QMatrix4x4>* transforms, int camera_id){
    ROS_WARN_COND(transforms->size() < cloud_matrices[camera_id]->size(), "Got less transforms than before!");
    ...
    QList<QMatrix4x4>* cloud_matrices_tmp = cloud_matrices[camera_id];
    cloud_matrices[camera_id] = transforms;
    ROS_DEBUG("New Cloud matrices size: %d", cloud_matrices[camera_id]->size());
    //clearAndUpdate();
    //FIXME: This should be replaced by a mutex'ed delete. Requires also to mutex all the read accesses
    delete cloud_matrices_tmp;
}

void GLViewer::addPointCloud(pointcloud_type * pc, QMatrix4x4 transform, int camera_id){
    ROS_INFO("pc pointer in addPointCloud: %p", pc);
    if(!pc->isOrganized() || ParameterServer::instance()->get<double>("squared_meshing_threshold") < 0){
      pointCloud2GLPoints(pc, camera_id);
    } else {
      pointCloud2GLStrip(pc, camera_id);
    }
    cloud_matrices[camera_id]->push_back(transform); //keep for later
    clearAndUpdate();
    //QApplication::processEvents(QEventLoop::ExcludeSocketNotifiers);
    std::string file_prefix = ParameterServer::instance()->get<std::string>("screencast_path_prefix");
    if(!file_prefix.empty()){
      QString filename;
      filename.sprintf("%s%.5d.png", file_prefix.c_str(), cloud_matrices[camera_id]->size());
...
void GLViewer::pointCloud2GLStrip(pointcloud_type * pc, int camera_id){
    ScopedTimer s(__FUNCTION__);
    ...
    glEndList();
    cloud_list_indices[camera_id].push_back(cloud_list_index);
...
void GLViewer::deleteLastNode(int camera_id){
    /*
  if(cloud_list_indices[0].size() <= 1){
    this->reset();
    return;
  }*/
    GLuint nodeId = cloud_list_indices[camera_id].back();
    cloud_list_indices[camera_id].pop_back();
    glDeleteLists(nodeId,1);

    cloud_list_indices[camera_id].clear();
    cloud_matrices[camera_id]->clear();
    edge_list_[camera_id].clear();
  /*
...
void GLViewer::pointCloud2GLPoints(pointcloud_type * pc, int camera_id){
    ScopedTimer s(__FUNCTION__);
...
void GLViewer::pointCloud2GLList(pointcloud_type const * pc, int camera_id){
    ScopedTimer s(__FUNCTION__);
    ROS_DEBUG("Making GL list from point-cloud pointer %p in thread %d", pc, (unsigned int)QThread::currentThreadId());
    GLuint cloud_list_index = glGenLists(1);
    if(!cloud_list_index) {
...
void GLViewer::reset(){
    for(int i = 0; i < camera_count_; i++){
        if(!cloud_list_indices[i].empty()){
          unsigned int max= cloud_list_indices[i].size() > feature_list_indices.size()? cloud_list_indices[i].back() : feature_list_indices.back();
          glDeleteLists(1,max);
        }
        cloud_list_indices[i].clear();
        cloud_matrices[i]->clear();
        edge_list_[i].clear();
    }
...
void GLViewer::setEdges(const QList<QPair<int, int> >* edge_list, int camera_id){
  //if(edge_list_) delete edge_list_;
    edge_list_[camera_id] = *edge_list;
  delete edge_list;
}

void GLViewer::drawEdges(int camera_id){
  if(edge_list_[camera_id].empty()) return;
  //glEnable (GL_LINE_STIPPLE);
  //glLineStipple (1, 0x1111);
  glBegin(GL_LINES);
  glLineWidth(12);
  for(int i = 0; i < edge_list_[camera_id].size(); i++){
    int id1 = edge_list_[camera_id][i].first;
    int id2 = edge_list_[camera_id][i].second;
    float x,y,z;
    if(cloud_matrices[camera_id]->size() > id1 && cloud_matrices[camera_id]->size() > id2){//only happens in weird circumstances
      ...
      x = (*cloud_matrices[camera_id])[id1](0,3);
      y = (*cloud_matrices[camera_id])[id1](1,3);
      z = (*cloud_matrices[camera_id])[id1](2,3);
      glVertex3f(x,y,z);
      x = (*cloud_matrices[camera_id])[id2](0,3);
      y = (*cloud_matrices[camera_id])[id2](1,3);
      z = (*cloud_matrices[camera_id])[id2](2,3);
      glVertex3f(x,y,z);
    }
    //This happens if the edges are updated faster than the poses/clouds
    else ROS_WARN("Not enough cloud matrices (%d) for vertex ids (%d and %d)", cloud_matrices[camera_id]->size(), id1, id2);
  }
  glEnd();
  //glDisable (GL_LINE_STIPPLE);
  ...
  if(show_tfs_){
    ...
    for(int i = 0; i < edge_list_[camera_id].size(); i++){
      ...
      int id2 = edge_list_[camera_id][i].second;
      if(cloud_matrices[camera_id]->size() > id1 && cloud_matrices[camera_id]->size() > id2){//only happens in weird circumstances
        Eigen::Vector3f x1((*cloud_matrices[camera_id])[id1](0,3), (*cloud_matrices[camera_id])[id1](1,3), (*cloud_matrices[camera_id])[id1](2,3));
        Eigen::Vector3f x2((*cloud_matrices[camera_id])[id2](0,3), (*cloud_matrices[camera_id])[id2](1,3), (*cloud_matrices[camera_id])[id2](2,3));
