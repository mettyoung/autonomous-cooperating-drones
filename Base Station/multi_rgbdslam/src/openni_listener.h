...
class OpenNIListener : public QObject {
  ...
  Q_OBJECT
  Q_SIGNALS:
    ///Connect to this signal to get up-to-date rgb and depth images from the listener
    void newRGBDImage(QImage rgb, QImage depth, int camera_id);
    void newFeatureFlowImage(QImage, int camera_id);
  ...
  protected:
    ...
    QImage cvMat2QImage(const cv::Mat& image, unsigned int idx, int camera_id);
    //! Create a QImage from several one-channel images.
    ///The QImage stores its data in the rgba_buffers_ indexed by idx (reused/overwritten each call)
    QImage cvMat2QImage(const cv::Mat& channel1, const cv::Mat& channel2, const cv::Mat& channel3, unsigned int idx, int camera_id);
    //!Retrieve the transform between the lens and the base-link at capturing time

    //COMMENTED FOR TESTING

    void retrieveTransformations(std_msgs::Header depth_header, Node* node_ptr, int camera_id);

    //!Call processNode either regularly or as background thread
    void callProcessing(cv::Mat gray_img, Node* node_ptr, int camera_id);

    //!processNode is called by cameraCallback in a separate thread and after finishing visualizes the results
    void processNode(Node* new_node, int camera_id);
