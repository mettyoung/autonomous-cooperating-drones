/* Edited by Emmett N. Young (4-22-14)
   1. void GraphManager::sendAllCloudsImpl(): increments the GraphManager::mLimit member if the node
   has not yet been sent. Otherwise, it will not be incremented. As a result, this enables us to only
   send nodes that have not yet been sent. For your information, graph_ is a typedef of std::map<int,Node*>.
   I have observed that the key increments by 1 in each new node added. I used this fact to my advantage
   in order to make the sending of PointClouds and /tf incremental since octomap server can build maps
   incrementally also.
   2. void GraphManager::sendAllClouds(): tests if the GraphManager::mIsExecuting member is true, then
   it will not create a new thread. Otherwise, it will do create and sets the mIsExecuting member to true.
   3. void GraphManager::sendAllCloudsImpl(): sets the mIsExecuting member to false at the end of the function.
...
void GraphManager::sendAllClouds(bool threaded){
    ...
    if (f1_.isFinished() && !isRestrictedSendAllCloudsImpl_)
        f1_ = QtConcurrent::run(this, &GraphManager::sendAllCloudsImpl);
    if (f2_.isFinished())
        f2_ = QtConcurrent::run(this, &GraphManager::sendLoopClosure2DMap);
}

tf::StampedTransform GraphManager::computeFixedToBaseTransform(Node* node, bool invert, int camera_id)
{
    ...
    printTransform("init_base_pose_", init_base_pose_[camera_id]);

//    tf::Transform world2base = init_base_pose_[camera_id]*base2points*computed_motion*base2points.inverse();
    tf::Transform world2base = base2points*computed_motion*base2points.inverse();
...
void GraphManager::sendLoopClosure2DMap()
{
    ScopedTimer s(__FUNCTION__);

    ros::NodeHandle n(ros::this_node::getNamespace()+ ros::this_node::getName() + "/config/");
    std::vector<std::string> camera_link(camera_count_);
    std::vector<std::string> camera_rgb_optical_frame(camera_count_);
    for (int i = 0; i < camera_count_; i++)
    {
        camera_link[i] = ParameterServer::instance()->get<std::string>("base_frame_name" +
                                                                       boost::lexical_cast<std::string>(i));
        camera_rgb_optical_frame[i] = ParameterServer::instance()->get<std::string>("base_frame_id" +
                                                                                    boost::lexical_cast<std::string>(i));
    }

    std::vector<bool> isTransformAvailable(camera_count_, true);

    std::vector<tf::StampedTransform> transform(camera_count_);
    for (int i = 0; i < camera_count_; i++)
    {
        try {
            tf_.lookupTransform(camera_link[i], camera_rgb_optical_frame[i], ros::Time(0), transform[i]);
        } catch(tf::TransformException& ex){
            ROS_ERROR_STREAM( "Err..Transform error of sensor data: " << ex.what() << ", quitting callback" <<
                              camera_link[i] << "->" << camera_rgb_optical_frame[i]);
            isTransformAvailable[i] = false;
        }
    }
    ...
    for (int i = 0; i < graph_.size(); i++)
    {
        ...
        // If transformation is unavailable from before, do not continue then.
        if (isTransformAvailable[node->camera_id_] != true)
            continue;
        ...
        tf::StampedTransform base_to_fixed = this->computeFixedToBaseTransform(node, false, node->camera_id_);

        // Added by Emmett
        // T(map->camera_link) * T(camera_link->camera_rgb_optical_frame)
        tf::Transform t1 = base_to_fixed;
        tf::Transform t2 = transform[node->camera_id_];
        base_to_fixed.mult(t1, t2);

        if (pOct_ != &oct1_)
            // If it is still not the last element, only buff and do not publish.
            if (i < graph_.size()-1)
            {
                if (node->camera_id_ == 0)
                {
                    n.param("occupancy_min_z0", min, min);
                    n.param("occupancy_max_z0", max, max);
                    oct1_.setFilters(min, max);
                }
                else
                {
                    n.param("occupancy_min_z1", min, min);
                    n.param("occupancy_max_z1", max, max);
                    oct1_.setFilters(min, max);
                }
                oct1_.insertCloudCallback(node, base_to_fixed);
            }
            else
            {
                send2DMapMutex_.lock(); // We must lock the mutex in order for the other
                // thread to not alter the variables which this thread
                // will alter.
                isRestrictedSendAllCloudsImpl_ = true;
                // potential threading problems to occur.
                oct2_.reset();          // Clear the other map
                if (node->camera_id_ == 0)
                {
                    n.param("occupancy_min_z0", min, min);
                    n.param("occupancy_max_z0", max, max);
                ...
                oct1_.insertCloudCallback(node, base_to_fixed, true);
                pOct_ = &oct1_;         // Bring the pointer back to the other octomap2DServer.
                send2DMapMutex_.unlock(); // Grant the other thread to access.
                isRestrictedSendAllCloudsImpl_ = false;
                break;  // THIS IS VERY IMPORTANT. Since it is not rest assured that after this loop,
                // the current graph may have already expanded.
            }
        else
            if (i < graph_.size()-1)
            {
                ...
                oct2_.insertCloudCallback(node, base_to_fixed);
            }
            else
            {
                ...
                oct2_.insertCloudCallback(node, base_to_fixed, true);
                pOct_ = &oct2_;
                send2DMapMutex_.unlock();
                isRestrictedSendAllCloudsImpl_ = false;
                break;
            }
//        std::cerr << "[LoopClosure, cameraID]: " << j++ << "," << node->camera_id_ << std::endl;
...
//    std::cerr << min << "," << max << std::endl;
    ...
    for (int i = 0; i < graph_.size(); i++)
    {
        ...
        // Emmett: The mLimit holds the value of index of the last node sent. Given the circumstance that
        // i (the current index) is less than the index of the last node sent means that the current iterator
        // has already been sent. Otherwise, increment mLimit by 1 if the current iterator has not yet been sent.
        // Thereby, setting the new index of the last node sent contained in the mLimit variable.
        if (i < mLimit)
            continue;
        else
            mLimit++;
        ...
        pOct_->insertCloudCallback(node, base_to_fixed, true);

        send2DMapMutex_.unlock();
        // **** Ends here
        QString message;
        Q_EMIT setGUIInfo(message.sprintf("Sending pointcloud and map transform (%i/%i) on topics %s and /tf", i, (int)graph_.size(), ParameterServer::instance()->get<std::string>("individual_cloud_out_topic").c_str()) );
    }
...
void GraphManager::saveIndividualCloudsToFile(QString file_basename)
{
    ...
    for (graph_it it = graph_.begin(); it != graph_.end(); ++it){
      ...
      *//*
        ...
        world2base = init_base_pose_[camera_id]*base2points*pose*base2points.inverse();
...
tf::StampedTransform GraphManager::stampedTransformInWorldFrame(const Node* node, const tf::Transform& computed_motion, int camera_id) const
{
    std::string idString = boost::lexical_cast<std::string>(camera_id);
    std::string fixed_frame = ParameterServer::instance()->get<std::string>("fixed_frame_name");
    std::string base_frame  = ParameterServer::instance()->get<std::string>("base_frame_name"+idString);
...
//    tf::Transform world2base = init_base_pose_[camera_id]*base2points*computed_motion*base2points.inverse();
    ...
    // Emmett: Added the line below. The commented line is the original.
    //    return tf::StampedTransform(world2base.inverse(), base2points.stamp_, base_frame, fixed_frame);
    return tf::StampedTransform(world2base, base2points.stamp_, fixed_frame, base_frame);
}
...
void GraphManager::drawFeatureFlow(int camera_id, cv::Mat& canvas, cv::Scalar line_color,
                                   cv::Scalar circle_color)
{
    ROS_INFO("START: drawFeatureFlow");
    ...
    ROS_DEBUG("Number of features to draw: %d", (int)curr_best_result_[camera_id].inlier_matches.size());
    ...
    if(graph_.empty()) {
        ...
        ROS_INFO("END: drawFeatureFlow");
        return;
    } else if(cam_ids_to_v_ids_[camera_id].size() == 1 || curr_best_result_[camera_id].edge.id1 == -1 ) {//feature flow is only available between at least two nodes
        Node* newernode = graph_[prev_node_[camera_id]];
        cv::drawKeypoints(canvas, newernode->feature_locations_2d_, canvas, cv::Scalar(255), 5);
        ROS_INFO("END: drawFeatureFlow");
        return;
    }

    Node* earliernode = graph_[curr_best_result_[camera_id].edge.id1];//graph_.size()-2; //compare current to previous
    Node* newernode = graph_[curr_best_result_[camera_id].edge.id2];
    if(earliernode == NULL){
        if(newernode == NULL ){ ROS_ERROR("Nullpointer for Node %u", (unsigned int)graph_.size()-1); }
        ROS_ERROR("Nullpointer for Node %d", curr_best_result_[camera_id].edge.id1);
        curr_best_result_[camera_id].edge.id1 = 0;
        ROS_INFO("END: drawFeatureFlow");
        return;
    ...
    canvas+=tmpimage;
    for(unsigned int mtch = 0; mtch < curr_best_result_[camera_id].inlier_matches.size(); mtch++) {
        cv::Point2f p,q; //TODO: Use sub-pixel-accuracy
        unsigned int newer_idx = curr_best_result_[camera_id].inlier_matches[mtch].queryIdx;
        unsigned int earlier_idx = curr_best_result_[camera_id].inlier_matches[mtch].trainIdx;
