...
bool MissionPlanner::requestForfrontier_cb(merry_navigation_stack::QueryForFrontiersRequest &req, merry_navigation_stack::QueryForFrontiersResponse &res)
{
    ...
    for (int i = 0; i < 2; i++)
        if (isLocalizationValid[i])
        {
            ...
            if(!mapListener.inflatedMap().worldToMap(sTransform[i].getOrigin().x(), sTransform[i].getOrigin().y(),
                                                     my, mx))
                break;
            Dijkstra::findNearestFreeCellInBlack(mapListener.inflatedMap().gridMap(), cv::Point2i(mx, my),
                                          newStart);
            if (req.id == i)
            {
                log_[req.id] << "Original[" << (int)req.id << "] = " << mx << "," << my << "\n";
                log_[req.id] << "Relocated[" << (int)req.id << "] = " << newStart.x << "," << newStart.y << "\n";
                newPose = newStart;
            }

            Dijkstra::computeCostVector(mapListener.inflatedMap().gridMap(), newStart, relocatedCOG, costVector);
    ...
    if (i < relocatedCOG.size())
    {
        ...
        mapListener.inflatedMap().mapToWorld(newPose.y, newPose.x, wx, wy);
...
//        res.currentPose.pose.position.y = sTransform[req.id].getOrigin().y();
        res.currentPose.pose.position.x = wx;
        res.currentPose.pose.position.y = wy;
