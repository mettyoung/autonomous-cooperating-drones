        ...
        for (int x = currentCell.id_.x-1; x <= currentCell.id_.x+1; x++)
            for (int y = currentCell.id_.y-1; y <= currentCell.id_.y+1; y++)
            {
                ...
                if (checkIfNotOutOfBounds(cv::Point2i(x, y), gridMap.rows, gridMap.cols) &&
                        get_cell_at(gridMap, x, y) == Dijkstra::FREE_CELL)
                {
                                  ...
                                  std::vector<std::pair<cv::Point2i, cv::Point2i > > &discretizedPath)
{
    ...
    cv::Point2i p1, p2;
    p1 = cv::Point2i(path[0].x, path[0].y);
    p2 = cv::Point2i(path[1].x, path[1].y);
    ...
    for (int i = 1; i < path.size(); i++)
    {
        ...
        if (p2.x != p1.x)
        {
            ...
            for (int x = min.x; x < max.x; x++)
                for (int y = min.y; y < max.y; y++)
                {
                    ...
                    //
                    if (std::abs(y - (slope*x+b)) < 1 && gridMap.data[y*gridMap.cols+x] != Dijkstra::FREE_CELL)
                        obstacle_found = true;
                }
        }
        ...
        else
        {
            ...
            for (int y = y_min; y < y_max; y++)
                if (gridMap.data[y*gridMap.cols+p1.x] != Dijkstra::FREE_CELL)
                    obstacle_found = true;
        }
...
void Dijkstra::findNearestFreeCell(const cv::Mat& gridMap, const cv::Point2i &start, cv::Point2i &end, int numberOfFreeCells)
{
    ...
    while(!working.empty())
    {
        ...
        if (gridMap.data[currentCell.id_.y*gridMap.cols+currentCell.id_.x] == Dijkstra::FREE_CELL
                && n++ == numberOfFreeCells)
        {
        ...
        for (int x = currentCell.id_.x-1; x <= currentCell.id_.x+1; x++)
            for (int y = currentCell.id_.y-1; y <= currentCell.id_.y+1; y++)
            {
                ...
                if (checkIfNotOutOfBounds(cv::Point2i(x, y), gridMap.rows, gridMap.cols) &&
                        (get_cell_at(gridMap, x, y) == Dijkstra::UNKNOWN_CELL ||
                        get_cell_at(gridMap, x, y) == Dijkstra::FREE_CELL))
                {
                    double cost = currentCell.cost_ + sqrt((x-currentCell.id_.x)*(x-currentCell.id_.x)
                                                           +(y-currentCell.id_.y)*(y-currentCell.id_.y));
...
void Dijkstra::findNearestFreeCellInBlack(const cv::Mat& gridMap, const cv::Point2i &start, cv::Point2i &end, int numberOfFreeCells)
{
