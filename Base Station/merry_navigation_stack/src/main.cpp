...
QueryPose queryPose(2); // 2 second timeout
...
int main(int argc, char **argv)
{
    ...
    double timeout = 2;
    int freqOfSendAll = 5;
    n.param(ros::this_node::getNamespace() + ros::this_node::getName() + "/pose_timeout", timeout, timeout);
    n.param(ros::this_node::getNamespace() + ros::this_node::getName() + "/freq_map", freqOfSendAll, freqOfSendAll);

    /// Set QueryPose frequency
    queryPose.setTimeout(timeout);
    ...
    serviceClient2->setFreqOfSendAll(freqOfSendAll);
    ...
    QObject::connect(localPlanner0, SIGNAL(setCommandQuad(double, double)),
                     &w, SLOT(setCommandQuad0(double,double)));
    QObject::connect(localPlanner1, SIGNAL(setCommandQuad(double, double)),
                     &w, SLOT(setCommandQuad1(double,double)));
    QObject::connect(globalPlanner0, SIGNAL(hoverQuad()), localPlanner0, SLOT(hoverQuad()));
    QObject::connect(globalPlanner1, SIGNAL(hoverQuad()), localPlanner1, SLOT(hoverQuad()));
    QObject::connect(localPlanner0, SIGNAL(requestNewPath()), globalPlanner0, SLOT(on_reqBtn_clicked()));
    QObject::connect(localPlanner1, SIGNAL(requestNewPath()), globalPlanner1, SLOT(on_reqBtn_clicked()));
    QObject::connect(&w, SIGNAL(toggleAutonomous()), localPlanner0, SLOT(toggleAutonomous()));
    QObject::connect(&w, SIGNAL(toggleAutonomous()), localPlanner1, SLOT(toggleAutonomous()));
    /// Query Pose
    ...
    QObject::connect(&mapListener.log_, SIGNAL(logConsoleToDisplayImage(QImage,QString,uint)),
                     &w, SLOT(writeToDisplayImage(QImage,QString,uint)));
